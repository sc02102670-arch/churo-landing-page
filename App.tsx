...
// Header, Hero 등 존재하지 않는 components 폴더 관련 import를 모두 제거했습니다.

const App: React.FC = () => {
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  // selectedProgram 관련 타입 에러를 방지하기 위해 타입을 any로 임시 처리하거나 관련 로직을 최소화합니다.
  ...
  const handleSearchResultClick = (program: any) => {
  ...
  return (
    <div className="relative selection:bg-brand-green selection:text-white">
      ...
      {/* 존재하지 않는 Header 부품을 제거했습니다. */}
      
      <main>
        {/* Hero, About 부품을 제거했습니다. */}
        <section id="program" className="bg-raw-gray py-24">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* ProgramGrid 부품을 제거하고 목록만 텍스트로 표시하거나 비워둡니다. */}
                <div className="text-center font-bold">프로그램 목록 준비 중...</div>
            </div>
        </section>
        {/* ReferenceSection, Contact 부품을 제거했습니다. */}
      </main>
